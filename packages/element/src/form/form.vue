<template>
  <el-form
    ref="formRef"
    v-bind="formProps"
    style="display: flex; flex-wrap: wrap"
  >
    <div
      style="width: 50%; padding: 0 10px"
      v-for="(item, index) in items"
      :key="item.label + '-' + index"
      :style="item.style"
    >
      <el-form-item
        v-if="item.type === 'form-item'"
        v-bind="item"
        :label="item.label + ':'"
        style="width: 100%"
        :prop="item.name"
      >
        <div style="display: flex; align-items: center; width: 100%">
          <div style="flex: 1">
            <component
              :is="item.component.name"
              v-model="formProps.model[item.name]"
              v-bind="item.component.props"
              v-on="item.component.listeners"
            ></component>
          </div>
          <div style="width: 24px; margin-left: 4px">
            <ElPopover v-if="item.tooltip" trigger="click">
              <template #reference>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                >
                  <g fill="none">
                    <path
                      d="m12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035q-.016-.005-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093q.019.005.029-.008l.004-.014l-.034-.614q-.005-.018-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z"
                    />
                    <path
                      fill="currentColor"
                      d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12S6.477 2 12 2m0 14a1 1 0 1 0 0 2a1 1 0 0 0 0-2m0-9.5a3.625 3.625 0 0 0-3.625 3.625a1 1 0 1 0 2 0a1.625 1.625 0 1 1 2.23 1.51c-.676.27-1.605.962-1.605 2.115V14a1 1 0 1 0 2 0c0-.244.05-.366.261-.47l.087-.04A3.626 3.626 0 0 0 12 6.5"
                    />
                  </g>
                </svg>
              </template>
              <div v-html="item.tooltip.text"></div>
            </ElPopover>
          </div>
        </div>
      </el-form-item>
      <div v-if="item.type === 'display-item'">
        <component
          :is="item.component.name"
          v-model="formProps.model[item.name]"
          v-bind="item.component.props"
          v-on="item.component.listeners"
        ></component>
      </div>
    </div>
  </el-form>
</template>

<script lang="ts" setup>
import {
  ElForm,
  ElFormItem,
  FormProps,
  FormItemProps,
  ElButton,
  ElPopover,
} from 'element-plus'
import { reactive, type Component, watch, ref } from 'vue'

type Props = {
  formProps: Partial<FormProps>
  items: (Partial<FormItemProps> & {
    style?: any
    type: 'form-item' | 'display-item'
    name: string
    tooltip?: {
      text: string
    }
    component: {
      name: Component
      props?: any
      listeners?: any
    }
  })[]
}

const _props = defineProps<Props>()

const _emits = defineEmits<{ search: []; reset: [] }>()

const formRef = ref(null)

defineExpose({
  validate() {
    return new Promise(resolve => {
      debugger
      formRef.value.validate(valid => {
        if (valid) {
          resolve(true)
        } else {
          resolve(false)
        }
      })
    })
  },
})

function createProxy() {
  const formData = _props.formProps.model
  const proxy = {}
  Object.keys(formData).map(key => {
    Object.defineProperty(proxy, key, {
      get() {
        return formData[key]
      },
      set(newValue) {
        formData[key] = newValue
      },
    })
  })
}
</script>
